...
class SocketService {
    ...
    public subscribe(destination: string, callback: (payload: any) => void): StompSubscription {
        if (!this.client || !this.isConnected) {
            throw new Error('Cannot subscribe! Frontend does not connected to the socket');
        }
