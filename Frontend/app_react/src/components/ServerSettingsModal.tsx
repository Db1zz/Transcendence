...
import { X, Shield, Info, Users, Trash2, Save, AlertCircle, Plus } from "lucide-react";
import { useRoles, Role } from "../hooks/useRoles";
import { usePermissions, PERMISSION_LIST, PERMISSION_FLAGS } from "../hooks/usePermissions";
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm px-4 transition-all">
      <div className="bg-[#f2ece9] rounded-xl shadow-2xl w-full max-w-4xl h-[80vh] flex overflow-hidden border border-[#d3c5bd]">
        <div className="w-1/3 bg-[#e8deda]/50 border-r border-[#d3c5bd] p-6 flex flex-col gap-2">
          ...
          <button onClick={() => setActiveTab("overview")} className={`flex items-center gap-3 px-3 py-2.5 rounded-lg font-bold text-sm transition-all ${activeTab === "overview" ? "bg-brand-green text-brand-beige shadow-sm" : "text-gray-600 hover:bg-[#d3c5bd]/40 hover:text-gray-900"}`}>
          ...
          <div className="p-8 h-full overflow-y-auto scrollbar-hide">
            {errorMessage && (
              <div className="mb-6 flex items-center gap-3 bg-red-50 border-l-4 border-red-500 text-red-700 px-4 py-3 rounded shadow-sm relative animate-in slide-in-from-top-2">
                ...
                <span className="block sm:inline text-sm font-medium">{errorMessage}</span>
                <button onClick={() => setErrorMessage(null)} className="absolute top-3 right-3 text-red-400 hover:text-red-700">
                  <X size={16} />
                </button>
              </div>
            )}
            ...
            {activeTab === "roles" && canManageRoles && (
              <div className="flex flex-col h-full animate-in fade-in">
                ...
                <form onSubmit={handleCreateRole} className="bg-white border border-gray-200 p-6 rounded-xl shadow-sm mb-8 shrink-0 relative overflow-hidden">
                  <div className="absolute top-0 left-0 w-1 h-full bg-brand-green"></div>
                  <h3 className="font-bold text-gray-800 mb-4 flex items-center gap-2">
                    <Plus size={18} className="text-brand-green"/> Create New Role
                  </h3>    
                  <div className="flex gap-3 mb-6">
                    <input 
                      ...
                      value={newRoleName}
                      onChange={(e) => { setNewRoleName(e.target.value); setErrorMessage(null); }}
                    ...
                    />
                    <button type="submit" disabled={!newRoleName.trim()} className="bg-brand-green text-brand-beige px-6 py-2.5 rounded-lg font-bold hover:bg-brand-brick transition-all disabled:opacity-50 disabled:cursor-not-allowed shadow-sm">
                      Create Role
                    </button>
                  </div>
                  <div className="grid grid-cols-2 gap-y-3 gap-x-6 bg-gray-50 p-4 rounded-lg border border-gray-100">
                    {PERMISSION_LIST.map((flag) => {
                      const hasPerm = (newRolePermissions & flag.value) === flag.value;
                      return (
                        <label key={flag.value} className="flex items-center gap-3 text-sm text-gray-700 cursor-pointer group">
                          <div className={`w-5 h-5 rounded border flex items-center justify-center transition-colors ${hasPerm ? 'bg-brand-green border-brand-green' : 'bg-white border-gray-300 group-hover:border-brand-green/50'}`}>
                            {hasPerm && <X size={14} className="text-white rotate-45" style={{ transform: 'rotate(0deg)'}} />}
                          </div>
                          <input 
                            type="checkbox" 
                            checked={hasPerm}
                            onChange={() => setNewRolePermissions(togglePermission(newRolePermissions, flag.value))}
                            className="hidden"
                          />
                ...
                <div className="flex-1 overflow-y-auto pr-2 pb-4">
                  <h3 className="font-bold text-gray-800 mb-4 flex items-center justify-between">
                    ...
                    <span className="bg-brand-green/10 text-brand-green py-0.5 px-2.5 rounded-full text-xs">{roles.length} Roles</span>
                  </h3>  
                  <div className="space-y-3">
                    {roles.length === 0 ? (
                      <div className="text-center py-10 bg-white rounded-xl border border-dashed border-gray-300">
                        <Shield size={40} className="mx-auto text-gray-300 mb-3" />
                        <p className="text-gray-500 font-medium">No custom roles created yet.</p>
                      </div>
                    ) : (
                      roles.map(role => (
                        <div key={role.id} className="group flex flex-col bg-white border border-gray-200 p-4 rounded-xl shadow-sm hover:shadow-md transition-all">
                          {editingRoleId === role.id ? (
                            <div className="flex flex-col gap-4 animate-in fade-in">
                              <div className="flex items-center gap-3">
                                <input 
                                  type="text" 
                                  value={editRoleName}
                                  onChange={(e) => { setEditRoleName(e.target.value); setErrorMessage(null); }}
                                ...
                                />
                                <button onClick={handleUpdateRole} className="p-2 bg-brand-green text-white rounded-lg hover:bg-brand-brick transition-all shadow-sm flex items-center gap-1.5 px-4 font-bold text-sm">
                                  <Save size={16} /> Save
                                </button>
                                <button onClick={() => setEditingRoleId(null)} className="p-2 bg-gray-100 text-gray-600 rounded-lg hover:bg-gray-200 transition-all font-medium text-sm px-4">
                                  Cancel
                                </button>
                              </div>
                              <div className="grid grid-cols-2 gap-3 bg-gray-50 p-4 rounded-lg border border-gray-100">
                                {PERMISSION_LIST.map((flag) => {
                                  const hasPerm = (editRolePermissions & flag.value) === flag.value;
                                  return (
                                    <label key={flag.value} className="flex items-center gap-3 text-sm text-gray-700 cursor-pointer group">
                                      <div className={`w-5 h-5 rounded border flex items-center justify-center transition-colors ${hasPerm ? 'bg-brand-green border-brand-green' : 'bg-white border-gray-300 group-hover:border-brand-green/50'}`}>
                                        {hasPerm && <X size={14} className="text-white" style={{ transform: 'rotate(0deg)'}} />}
                                      </div>
                                      <input 
                                        type="checkbox" 
                                        checked={hasPerm}
                                        onChange={() => setEditRolePermissions(togglePermission(editRolePermissions, flag.value))}
                                        className="hidden"
                                      />
                                      <span className="font-medium">{flag.label}</span>
                                    </label>
                            ...
                            <div className="flex items-start justify-between gap-4">
                              ...
                              >
                                <p className="font-bold text-brand-green text-lg mb-1">{role.name}</p>
                                <div className="flex flex-wrap gap-1.5">
                                  {getActivePermissions(role.permissions).map((p) => (
                                    <span key={p.value} className="px-2.5 py-0.5 bg-brand-green/10 text-brand-green border border-brand-green/20 rounded-full text-[10px] font-bold uppercase tracking-wider">
                                      {p.label}
                                    </span>
                                  ))}
                                  {role.permissions === 0 && (
                              ...
                              <button 
                                onClick={async (e) => {
                                  ...
                                  catch (err: any) { setErrorMessage(err.response?.data?.error || "Failed to delete role"); }
                                }}
              ...
              <div className="flex flex-col h-full animate-in fade-in">
                ...
                <div className="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden flex-1 flex flex-col">
                  ...
                  <div className="overflow-y-auto p-2 space-y-1">
                    {members.map((member) => (
                      <div key={member.id} className="grid grid-cols-12 gap-4 items-center p-2 hover:bg-gray-50 rounded-lg transition-colors group">
                        <div className="col-span-5 flex items-center gap-3">
                          <img 
                            src={member.user.picture || "https://i.pinimg.com/1200x/c4/a4/36/c4a4365f7c98dc3b4b26fbad20da527d.jpg"} 
                            alt={member.user.username} 
                            className="w-8 h-8 rounded-full border border-gray-300 object-cover"
                          />
                          <div>
                            <p className="font-bold text-gray-800 text-sm">{member.user.displayName || member.user.username}</p>
