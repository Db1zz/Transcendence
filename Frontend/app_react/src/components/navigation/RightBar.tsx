...
export const RightBar: React.FC<RightBarProps> = ({ children }) => {
  ...
  return (
    <div className="h-full rounded-r-lg p-4 border-brand-green relative overflow-hidden flex flex-col">
      <div
        className="absolute inset-0 bg-cover bg-center"
        style={{ backgroundImage: `url(${bgSideBar})` }}
      />
      <div className="absolute inset-0 bg-brand-green opacity-90" />
      <div className="relative z-10 text-gray-800 font-semibold h-full flex flex-col">
        {children ? (
          children
        ) : (
