...
interface MobileNavBarProps {
	active?: "friends" | "chat" | "voice" | "server" | "friendsList" | "notifications";
	onNavigate?: (view: "friends" | "chat" | "voice" | "server" | "friendsList" | "notifications") => void;
	onMainClick?: () => void;
	onYouClick?: () => void;
}
	...
	return (
		<div className="md:hidden fixed bottom-0 left-0 right-0 z-50">
			<div className="mx-3 mb-3 rounded-xl shadow-lg flex items-center gap-3 px-3 py-2 bg-brand-green border border-brand-peach">
				...
				>
					<Bell size={20} className={`${notifActive ? "text-brand-beige" : "text-brand-beige"}`} />
				...
				>
					<div className="relative">
						<div className={`w-8 h-8 rounded-full overflow-hidden border-2 ${youActive ? "border-brand-beige" : "border-gray-800"}`}>
							<img src={user?.picture} alt={user?.name} className="w-full h-full object-cover" />
						</div>
						<span
							className={`absolute -bottom-0.5 -right-0.5 w-2.5 h-2.5 rounded-full border-2 border-white ${StatusColors[status as keyof typeof StatusColors] || "bg-gray-400"
								}`}
						/>
					</div>
