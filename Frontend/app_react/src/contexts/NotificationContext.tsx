...
export function NotificationProvider({ children, notifyServerAddr }: { children: React.ReactNode, notifyServerAddr: string }) {
	...
	const normalizeNotification = (item: BackendNotification): StoredNotification => ({
		...item,
		payload: parsePayload(item.payload),
	});

	const isMessageNotification = (item: Pick<BackendNotification, "etype">) => item.etype === "MESSAGE_CREATED";
	...
	const dismissNotifications = async (ids: string[]) => {
		...
		setNotifications((prev) => prev.filter((item) => !uniqueIds.includes(item.id)));
	...
	return (
		<NotificationContext.Provider value={{ getUnreadCount, markAsRead, latestToast, notifications, dismissNotification, dismissNotifications, setActiveTarget, setIncomingCall, incomingCall }}>
			{children}
