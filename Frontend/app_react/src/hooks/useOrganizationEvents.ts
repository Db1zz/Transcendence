...
export const useOrganizationEvents = (
      ...
      const sub = subscribe(destination, (payload: any) => {
        ...
        if (typeof event === "string") {
          try {
            ...
            console.error(
              `Failed to nested-parse event for org ${server.id}`,
              err,
            );
          }
        }
    ...
    return () => {
      activeSubscriptions.forEach((sub) => {
        try {
          ...
          console.warn(
            "STOMP target was already unsubscribed or disconnected",
            e,
          );
        }
      });
  ...
  const sendOrganizationAction = useCallback(
    (orgId: string, action: string, payload: any) => {
      if (!isConnected) {
  ...
  const sendToOrganization = useCallback(
    (orgId: string, payload: any) => {
      if (!isConnected) {
        console.warn("Cannot send action, socket is not connected.");
        return;
      }
